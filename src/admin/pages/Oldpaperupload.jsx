...
const notesSchema = yup.object({
  ...
  paper: yup.string().required("Please Notes pdf"),
});
...
const Notesupload = () => {
  ...
  console.log(errors);
  const uploadPapers = async (data, e) => {
   
    e.preventDefault();

    const Data = { ...data, paper: e.target.elements.paper.files[0] };
    setLoading(true);
    const response = await usePostFileApi("https://easy-notes-backend.onrender.com/api/v1/admin/upload-oldpapers", Data);
    ...
    <>
      ...
      <div className="bg-primary w-full h-full flex flex-col md:flex-row justify-around items-center px-10 overflow-auto flex-grow basis-1/2">
        ...
        <form
          ...
          onSubmit={handleSubmit(uploadPapers)}
        >
